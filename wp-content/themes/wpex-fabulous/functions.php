...
function getDatosCliente($id) {
    ...
    while ($dato = $datos->fetch_object()) {
        ...
        if (!empty($dato->rubro)) {
            $htmlDatos .= '<p><strong>Rubro</strong>: '.ucfirst(mb_strtolower($dato->rubro)).'</p>';
        }
        if (!empty($dato->email)) {
        ...
        if (!empty($direccion)) {
            $htmlDatos .= '<p> <strong>Dirección</strong>: '.$direccion.'</p>';
        }
        if (!empty($dato->otros_datos)) {
            $htmlDatos .= '<p> <strong>Otros datos de interés:</strong>: '.$dato->otros_datos.'</p>';
        }
...
function getNews() {
    ...
    $newsByCliente = $mysqli->query("SELECT area,folder,asunto,nombre_lista FROM listas  WHERE area ='14' order by id_lista DESC");
    ...
    while ($new = $newsByCliente->fetch_object()):
        echo "<li>";
        echo '<a href="http://www.boletinesinteligentes.com/app/data/'.$new->folder.'" target="_blank">Ver Newsletter '.utf8_encode($new->nombre_lista).'</a>';
        echo "</li>";
